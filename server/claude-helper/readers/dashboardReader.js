...
function applyRefreshLoopMeta(dashboard) {
  ...
  const gdeltWorker = refreshLoop?.last_result?.gdelt || {};
  const rssWorker = refreshLoop?.last_result?.rss || {};
  ...
  next.meta.refresh_loop = {
    ...
    worker: {
      gdelt: {
        selected_baskets: gdeltWorker.selected_baskets || [],
        next_basket: gdeltWorker.next_basket || null,
        baskets_per_cycle: gdeltWorker.baskets_per_cycle || null,
        rate_limited: Boolean(gdeltWorker.rate_limited),
        cooldown_until: gdeltWorker.cooldown_until || null
      },
      rss: {
        fetched: rssWorker.fetched ?? null,
        inserted: rssWorker.inserted ?? null,
        skipped: Boolean(rssWorker.skipped),
        reason: rssWorker.reason || null
      }
    }
  };
...
export function readDashboardAuditContext() {
  ...
  if (!bundle?.dashboard) {
    return {
      available: false,
      message: "No snapshot-backed dashboard is available for helper audit."
    };
  }

  return {
    available: true,
    source: "selected_snapshot_bundle",
    dashboard: applyRefreshLoopMeta(applySnapshotSelectionMeta(bundle.dashboard, selection))
  };
}
