...
export const mockDashboardResponse = {
  ...
  banner: { title: 'Live Signals Overlay', subtitle: 'Descriptive monitoring layer for event intensity, source mix, and correction-aware signal flow.', disclaimer: 'Non-predictive use only. This dashboard summarizes observed signals in the active window. It does not forecast outcomes or assign future probabilities.' },
  metrics: { cards: [
...
export const degradedDashboardResponse = {
  ...
  metrics: { cards: mockDashboardResponse.metrics.cards.map((c)=> c.key==='cross_source_coherence' ? { ...c, value:null, display:'Unavailable', delta_vs_prior_window:null, delta_direction:'flat', status:'degraded', sparkline:[] } : c ) },
...
export const mockMetricsHistory = { series: {
...
export const degradedMetricsHistory = mockMetricsHistory;
export const emptyMetricsHistory = { series: {} };

export const mockMetricExplain = { label:'Signal Velocity', definition:'Weighted cluster activity score for the active window.', drivers:[{cluster_id:'cl_lev_2201',headline:'Airspace restrictions under clarification',contribution:0.10}], caveats:['Signal velocity captures observed throughput, not forecast direction.'] };
export const degradedMetricExplain = { label:'Cross-Source Coherence', definition:'Explanation available with partial data.', drivers:[], caveats:['Feed delays can suppress explain drivers.'] };
export const emptyMetricExplain = { label:'Metric', definition:'No explain payload available in empty window.', drivers:[], caveats:['No active clusters in selected window.'] };
...
export { metricCardOrder, mockMetricsHistory as historyFallback, mockMetricExplain as explainFallback };
