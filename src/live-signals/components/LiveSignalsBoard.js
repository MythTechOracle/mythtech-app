...
export function renderLiveSignalsBoard(root, props) {
  ...
  if (!root) throw new Error('renderLiveSignalsBoard requires a root node.');

  const safeVm = vm ?? {
    header: { disclaimer: '', windowStart: '', windowEnd: '', windowLabel: '', generatedAt: '', refreshLabel: '', modeLabel: '' },
    statusBanner: { show: false, title: '', items: [] },
    metricCards: [],
    composition: { title: 'Signal Composition', rows: [] },
    eventTape: { title: 'Recent Event Tape', rows: [], empty: true },
  ...
  const notes = safeVm?.notes?.items ?? [];
  ...
  root.innerHTML = `
    <section class="panel grid">
      ...
      ${loading && !vm ? '<div class="small muted">Loading…</div>' : ''}
      ${error ? `<div class="banner degraded show"><strong>Load warning:</strong> ${error}</div>` : ''}
      <div class="banner"><strong>Non-predictive use only:</strong> ${safeVm.header.disclaimer ?? ''}</div>
      <div class="panel small muted"><div><strong>Window:</strong> ${safeVm.header.windowStart ?? ''} → ${safeVm.header.windowEnd ?? ''} (${safeVm.header.windowLabel ?? ''})</div><div><strong>Generated:</strong> ${safeVm.header.generatedAt ?? ''}</div><div>${safeVm.header.refreshLabel ?? ''} · ${safeVm.header.modeLabel ?? ''}</div></div>
      <div class="banner degraded ${safeVm.statusBanner.show?'show':''}">${safeVm.statusBanner.show?`<strong>${safeVm.statusBanner.title}</strong> ${safeVm.statusBanner.items.join(' · ')}`:''}</div>

      <div class="metrics" data-ls-metrics></div>
      <div class="cols-3">
        <div class="panel"><h3>${safeVm.composition.title}</h3><div data-ls-composition></div></div>
      ...
      <div class="cols-2">
        <div class="panel"><h3>${safeVm.notes.title}</h3><ul data-ls-notes></ul></div>
        <div class="panel"><h3>${safeVm.method.title}</h3><ul data-ls-method></ul></div>
      </div>
    </section>
  ...
  if(!metricCards.length) metrics.innerHTML='<div class="small muted">No metric cards available.</div>';
  ...
  if(!compositionRows.length) comp.innerHTML='<div class="small muted">No composition data.</div>';

  const normalizedHistory = history?.series ? history : { series: {} };
  const trends=root.querySelector('[data-ls-trends]');
  metricCards.forEach((card)=>{
    const series=normalizedHistory.series[card.key]||{baseline_24h_avg:'n/a',baseline_7d_avg:'n/a',points:card.sparkline};
  ...
  const counts={}; eventRows.forEach((r)=>{counts[r.region]=(counts[r.region]||0)+1;});
  ...
  const notesRoot=root.querySelector('[data-ls-notes]'); notes.forEach((i)=>{const li=document.createElement('li'); li.textContent=i; notesRoot.appendChild(li);});
  const methodRoot=root.querySelector('[data-ls-method]'); method.forEach((i)=>{const li=document.createElement('li'); li.textContent=i; methodRoot.appendChild(li);});
...
async function openExplain(root, card, explain) {
  const explainData = explain ?? await apiFetch(card.explainPath || '', explainFallback);
  ...
  (explainData.drivers || []).forEach((d)=>{ const tr=document.createElement('tr'); tr.innerHTML=`<td>${d.cluster_id||'—'}</td><td>${d.headline||'—'}</td><td>${d.contribution??'—'}</td>`; rows.appendChild(tr);});
  if (!(explainData.drivers || []).length) rows.innerHTML='<tr><td colspan="3">No metric drivers returned.</td></tr>';
  const caveats=root.querySelector('[data-ls-caveats]'); caveats.innerHTML=''; (explainData.caveats||[]).forEach((c)=>{ const li=document.createElement('li'); li.textContent=c; caveats.appendChild(li);});
